...
DETAILS = {
    ...
    "UC-A6": "An attacker must accomplish an objective while actively evading a blue team. The archetype is pre-positioning (Volt Typhoon, KV Botnet), where an actor quietly establishes and holds long-term access ahead of a future operation rather than striking immediately. The defining constraint is operational security: the operator relies on living-off-the-land techniques, avoids deploying detectable malware, and blends with legitimate administrative activity. Because blending in is only meaningful when there is realistic activity to blend with and realistic detection to evade, this is the most demanding objective; it exercises nearly every realism dimension at once.",
    "UC-D1": "A defender processes a stream of alerts and must separate true positives from false positives under realistic base rates. This is the counterpart to every attacker archetype, and it depends squarely on benign activity: there is nothing to triage unless malicious events sit within a noisy baseline of normal logins, administration, and traffic. Performance is measured by true-positive rate within a service-level window, mean time to detect, and the management of analyst fatigue and false-positive cost, none of which is testable against a clean, attack-only signal.",
